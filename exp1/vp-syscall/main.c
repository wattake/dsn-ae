...
int main()
{
	...
	log_inc = get_all_log_size();
	res = getpid();
	log_inc = get_all_log_size()-log_inc;
	printf("---- log increments -----\n");
	printf("%d\n", log_inc);
#if !CONFIG_LIBVAMPOS_SALS
	goto out;
#endif
	...
	/* measure increments of log entries */
	...
	log_inc = get_all_log_size();
	res = write(client, reply, sizeof(reply) - 1);
	log_inc = get_all_log_size()-log_inc;
	close(client);
	printf("---- log increments -----\n");
	printf("%d\n", log_inc);
	...
	log_inc = get_all_log_size();
	res = read(client, recvbuf, BUFLEN);
	log_inc = get_all_log_size()-log_inc;
	write(client, reply, sizeof(reply) - 1);
	close(client);
	printf("---- log increments -----\n");
